...
public class SplitSearch1  {
	...
	IntVar selectVariable(IntVar[] v) {
	    if (v.length != 0) {
			...
			if(v[0].min() == v[0].max()){	//if max and min in current domain are the same the domman cant be split furhter
	...
	public PrimitiveConstraint getConstraint() {
	   ...
	   return new XlteqC(var, value); //new constraint is now lover than value instea of equal
	}
    }
}
